...
/******** PREMIUM NUMBER**********/
const premium = ["6283144780782@s.whatsapp.net"
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('♥','white'),color('♥','red'),color('♥','white'),color('SQAN CODENYA!!','white'),color('OWNER','red'),color('ZAKIGANS','white'),color('XBOT','yellow'))
})
...
// ANTI LINK GRUP
			...
			if (args.length < 1) return reply(ind.wrongf())
					...
					case 'XBotgroup':
					  if (!isRegistered) return reply(ind.noregis())
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
					client.sendMessage(from, '*Zaki Gans GROUP*\n\nLink : _BELUM TERSEDIA_ ', text, { quoted: mek })
					break
                 case 'avengers':
                 if (!isRegistered) return reply(ind.noregis())
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
 ...
 if (!isOwner) return reply(mess.only.ownerB)
	...
	.then(() => {client.sendMessage(from, options, text)})
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh kontak Zaki😗 <•_•> ,Jangan Diubah Ya Kontaknya (*>*)',MessageType.text, { quoted: mek} )
					break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
